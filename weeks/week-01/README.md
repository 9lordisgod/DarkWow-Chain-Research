...
6. **Open — DarkFi side of the comparison.** The upstream characterisations (overlay‑DAG, `Money` monolith, token‑weighted DAO, SAFT allocations) are taken from DarkWow's *Differences from Upstream* page. **Week 2 candidate:** verify each against `darkrenaissance/darkfi` at the fork point.
